...
class LanguageModel(object):
    def __init__(self,params):
        ...
        self.input = tf.placeholder(tf.float32, shape=(self.batch_size, self.seq_len))
        # Build the RNN
        self.rnn = Embedding(self.vocab_size + 1, 128, input_length=seq_len)(self.input)
        ...
        self.labels = tf.placeholder(tf.int64, shape=(self.batch_size, self.seq_len))
        losses = []
        outputs = []
        for t in range(self.seq_len):
            rnn_t = rnn_output[t]
            y_t = tf.reshape(self.labels[:, t], shape=(self.batch_size, 1))
    ...
    def train_on_batch(self,X_batch,Y_batch):
        self.sess.run(feed_dict={input: X_batch, labels: Y_batch})
